...
const App2 = () => {
  ...
  // ----- Detect Keypress ----- //
  useEffect(() => detectKeypress(addTextLine, increaseIndent, decreaseIndent, handleBackspace, changeFocus))
  ...
  const handleBackspace = () => {
    console.log("handleBackspace, selectedId.current:", selectedId.current)
    console.log("textLines", textLines)
  ...
  const changeFocus = (direction) => {
    const currentSelectedIndex = textLines.findIndex(tl => tl._id === selectedId.current)
    let newSelected
    if (direction === 'up') {
      newSelected = currentSelectedIndex > 0 ? textLines[currentSelectedIndex - 1] : textLines[0]
    }
    if (direction === 'down') {
      newSelected = currentSelectedIndex < textLines.length - 1 ? textLines[currentSelectedIndex + 1] : textLines[textLines.length - 1]
    }
    selectNew(newSelected._id) //change selectedId.current
    setTextLines([...textLines])  //rerender
  }
  ...
  const handleClick = (id) => {
    ...
    // const newTextLines = [...textLines]
    // const selected = newTextLines.find(tl => tl._id === id)
    // selected.isInputSelected = true
    // setTextLines(newTextLines)
  }
  ...
  const deleteTextLines = (ids) => {
    ...
    const newSelectedId = newTextLines[newTextLines.length-1]._id
    selectNew(newSelectedId)
  ...
  const selectNew = (id) => {
    console.log("select new", id)
    console.log("typeof id", typeof (id))
    textLineRefs.current[id].focus()
    selectedId.current = id
    if (selectedItems.length > 0) {
      const selectedNodes = document.querySelectorAll('.ds-selected')
      selectedNodes.forEach(node => {
        node.classList.remove('ds-selected')
      })
    }
  }
  ...
  // ----- Drag Select ----- //
  // for drag selecting multiple
  // for single selecting (replaces onClick)
  ...
  useEffect(() => {
    ds.subscribe("callback", (e) => {
      if (e.items.length > 0) {
        if (e.items.length === 1) selectNew(e.items[0].getAttribute('data-info'))
    ...
    <div className="App" style={AppStyle}>
      ...
      {textLines.map((textLine, index) =>
        <TextLine
          ...
          isInputSelected={textLine._id === selectedId.current}
        //isDragSelected={selectedItems.includes(textLine._id)}
        />
      )}
